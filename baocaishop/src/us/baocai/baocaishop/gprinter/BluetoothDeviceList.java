		...
		if (pairedDevices.size() > 0) {
			...
			for (BluetoothDevice device : pairedDevices) {
				if (device.getName().equals("Gprinter")) {
					mPairedDevicesArrayAdapter.add("包菜打印机" + "\n"
							+ device.getAddress());
				} else if(device.getName()==null) {
					mPairedDevicesArrayAdapter.add("未知设备" + "\n"
							+ device.getAddress());
				}else {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothDevice.ACTION_FOUND.equals(action)) {
				...
				if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
					if (device.getName()!=null && device.getName().equals("Gprinter")) {
						mNewDevicesArrayAdapter.add("包菜打印机" + "\n"
								+ device.getAddress());
					} else if(device.getName()==null) {
						mNewDevicesArrayAdapter.add("未知设备" + "\n"
								+ device.getAddress());
					}  else {
